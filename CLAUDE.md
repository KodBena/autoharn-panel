...
|---|---|---|---|---|---|
| makespan-scheduler | library | `import: makespan_scheduler` (editable install at `/home/bork/w/vdc/1/experience/makespan-scheduler`, a sibling checkout — not a submodule of this repo — into the shared venv `~/w/vdc/venvs/generic`) | minimum-makespan schedule proof for jobs under resource-conflict (mutual exclusion), concurrency-cap, **and/or directed job-precedence** constraints, via OR-Tools CP-SAT | reach for ordering 3+ claimed/claimable work items whenever a resource conflict, a concurrency cap, **or real directed precedence (a `work_depends`/`blocks-close` edge)** is in play — pass precedence via each `Job`'s own `depends_on` field (first-class since sibling-repo commit `fc48782`/merge `196030d`, 2026-07-16, 90/90 tests — do **not** fake precedence as a shared resource, and do not trust any older guidance in this repo's own history claiming precedence isn't natively encodable, that claim is stale as of that commit); remaining caveat: the `batches` array is one *feasible* minimum-makespan assignment, not necessarily earliest-possible-start per job — the solver may place a job later than its deps strictly require if doing so doesn't raise the makespan (solver-tiebreak-dependent), so don't over-read batch membership as a could-not-have-started-earlier guarantee; discharge evidence = the emitted schedule JSON (`jobs[].start/end`, `batches`, `optimal` flag), ledgered with `--refs` to the decomposition it ordered | `blessed: ordering three or more claimed/claimable work items under dependency, precedence, or quota constraints` |

(ledger row id=969, supersedes row=397, `--refs row:960`; upstream repo: `KodBena/makespan-scheduler`)
