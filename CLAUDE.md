...
|---|---|---|---|---|---|
| makespan-scheduler | service | `import: makespan_scheduler.online` (`OnlineClient` in `client.py`, against a running `online_server.py` instance — ZMQ+Redis-backed; editable install at `/home/bork/w/vdc/1/experience/makespan-scheduler`, a sibling checkout — not a submodule of this repo — into the shared venv `~/w/vdc/venvs/generic`; branches `online-scheduling-service`/`active-orchestrator-interface` pushed to `origin`, **not yet merged to master, no live server instance deployed** as of this entry — the batch `scheduler.py` CLI/`schedule()` entry point remains the practical fallback until deployment) | reactive/online minimum-makespan scheduling with live task-lifecycle tracking — register a task set once, mark tasks active/done server-side as real-world execution actually progresses, pull the next ready task(s) reactively — same OR-Tools CP-SAT engine underneath, superseding the prior one-shot compute-then-execute batch model | **mandated**: driving execution of 3+ claimed/claimable work items under dependency, precedence, resource-conflict, or quota constraints, **once the online service is deployed and reachable** — until then, the batch CLI is a disclosed fallback, not a silent substitute. Practical application: as tasks finish, `mark_done` triggers a real recompute (the `drop_node`+`optimize` equivalent) and `next_tasks` reports what's newly dispatchable, so a task-block is no longer necessarily gated on *every* member of a prior block finishing before *any* of its own members can start — each cross-block dependency edge resolves individually as it clears, not on a whole-block boundary. The old batch-array feasible-not-earliest-possible caveat no longer applies the same way here, since `next_tasks` reports true current readiness, not a precomputed batch assignment. | `mandated: driving execution of three or more claimed/claimable work items under dependency, precedence, resource-conflict, or quota constraints, once the online service is deployed and reachable` |

(ledger row id=1150, supersedes row=969, `--refs row:1148`; upstream repo: `KodBena/makespan-scheduler`)
